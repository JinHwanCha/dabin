$(document).ready(function () {
    ...
    // localStorage에서 MBTI 유형 가져오기
    var mbtiType = localStorage.getItem('mbtiType');

    // 해당 유형의 데이터가 존재할 경우
    if (mbtiType && items[mbtiType]) {
        // .title .click 텍스트 업데이트
        $('.title .click').text(mbtiType.toUpperCase());

        // .title .click의 href 속성 업데이트
        $('.title .click').attr('href', '../find/find.html');

        // 해당 MBTI 항목으로 리스트 동적 생성
        var listHtml = '';
        items[mbtiType].forEach(function (item, index) {
            listHtml += '<li class="item item' + (index + 1) + '">' + item + '</li>';
        });
        $('.list').html(listHtml); // 리스트 업데이트
    } else {
        // MBTI 유형이 없을 경우 기본 설정
        $('.title .click').text('Unknown');
        $('.list').html('<li class="item">정보를 불러올 수 없습니다.</li>');
    }
});
